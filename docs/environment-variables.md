...
| `GITLAB_TOKEN` 或 `GITLAB_PRIVATE_TOKEN` | `git_work`、`git_catalog` | 条件必填 | GitLab Personal Access Token，访问私有 GitLab 仓库时必填 |
| `GITLAB_URL` | `git_work`、`git_catalog` | 可选 | 自定义 GitLab 实例 URL，默认：`https://gitlab.com/api/v4` |
...
如果 `git_work` 需要访问 GitHub、Gitee 或 GitLab 仓库：
...
**GitLab 仓库访问**：
```bash
export GITLAB_TOKEN="glpat-xxxxx"                     # 访问私有仓库时必填
# 或使用
export GITLAB_PRIVATE_TOKEN="glpat-xxxxx"             # 同上
# 自定义 GitLab 实例（可选）
export GITLAB_URL="https://your-gitlab-instance.com/api/v4"
```

**使用场景**：
- `git_work` 调用中包含 `gitlab_repos` 参数
- 访问私有仓库时必填
- 访问公开仓库时可选（但建议设置以避免 API 速率限制）
- 使用自定义 GitLab 实例时设置 `GITLAB_URL`

### `git_catalog` 工具

`git_catalog` 工具用于查询 GitHub、Gitee 或 GitLab 仓库和提交活动。
...
**使用场景**：
- `git_catalog` 调用中设置 `provider="gitlab"`
- 访问公开仓库时可选（但建议设置以避免 API 速率限制）
- 访问私有仓库时必填
- 使用自定义 GitLab 实例时设置 `GITLAB_URL`（默认：`https://gitlab.com/api/v4`）
...
### 场景 7：使用 `git_catalog` 查询 GitLab 仓库
...
- **缺少 GitLab Token**：
  - `git_work`：访问私有仓库时会失败，提示需要设置 `GITLAB_TOKEN` 或 `GITLAB_PRIVATE_TOKEN`
  - `git_catalog`（provider='gitlab'）：访问私有仓库时会失败，提示需要设置 `GITLAB_TOKEN` 或 `GITLAB_PRIVATE_TOKEN`
...
2. **使用最小权限原则**
   ...
   - GitLab Token 需要 `read_api` 权限（访问私有仓库）
...
- [git_catalog 工具说明](../README.md#-git_catalog-githubgiteegitlab-仓库目录查询)
